...
def lingrars(latitude, meteolist, plot):
    ...
    for i in range(fintim):
        ...
        dayl[i] = 0.5 * (1. + 2. * np.arcsin(np.tan(rad * latitude) * np.tan(decc[i])) / pi)
    ...
    if plot is True:
        ...
        plt.subplot(231)
        plt.plot(doy, np.divide(yielD[:len(doy)], 1000), 'g-')
        plt.title('Yield accumulated (T/ha)')
        plt.grid()

        plt.subplot(232)
        plt.plot(doy, tracu[:len(doy)], 'b-')
        plt.title('Transpiration accumulated (mm)')
        plt.grid()

        plt.subplot(236)
        plt.plot(doy, np.divide(tiller[:len(doy)], 100), 'r-')
        plt.title('Tiller (M#/ha)')
        plt.grid()

        plt.subplot(234)
        plt.plot(doy, LAI[:len(doy)], 'g-')
        plt.title('LAI (-)')
        plt.grid()

        plt.subplot(235)
        plt.plot(doy, evacu[:len(doy)], 'b-')
        plt.title('Evaporation accumulated (mm)')
        plt.grid()

        plt.subplot(233)
        plt.plot(doy, np.divide(grass[:len(doy)], 1000), 'r-')
